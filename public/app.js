...
async function calculate() {
  ...
  try {
    ...
    // Find the trading day closest to exactly 1 year ago
    const today = new Date();
    const oneYearAgo = new Date(today);
    oneYearAgo.setFullYear(today.getFullYear() - 1);
    const targetStr = fmtDate(oneYearAgo);

    // Closest date on or after one year ago
    ...
    // Use last available price as "today"
    const startPoint = prices[startIdx];
    const endPoint   = prices[prices.length - 1];

    // Slice the 1-year window for the chart
